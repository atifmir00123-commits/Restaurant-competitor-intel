...
const API_HEADERS = {
  ...
  "x-api-key": "claude-artifact",
  "anthropic-version": "2023-06-01",
  "anthropic-beta": "claude-artifact-2025-01-10"
};
...
Answer their questions based on this data. Be specific, actionable, and concise. Reference actual competitor names and dish names from the report when relevant. Format your answers in clear paragraphs — no excessive bullet lists unless it genuinely helps.`;
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      // For follow-up messages, include prior chat history
      if (messages.length > 0) {
        const history = messages.map(m => ({ role: m.role, content: m.content }));
        apiMessages[0].content = reportContext;
  ...
  return (
    <div style={{ marginTop:40 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
        ...
        <div>
          <div style={{ fontSize:13, fontWeight:700, color:"#fff", letterSpacing:"0.02em" }}>Ask About Your Report</div>
          <div style={{ fontSize:11, color:"#555" }}>Ask anything about your competitors, dishes, or strategy</div>
        </div>
      </div>

      {/* Suggested Questions — only shown before any chat */}
      {messages.length === 0 && (
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {SUGGESTED_QUESTIONS.map((q, i) => (
              ...
              onMouseEnter={e => { e.target.style.borderColor="rgba(232,255,71,0.4)"; e.target.style.color="#D4D0C8"; }}
        ...
        <div style={{ marginBottom:16, display:"flex", flexDirection:"column", gap:14, maxHeight:480, overflowY:"auto", paddingRight:4 }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start", flexDirection: m.role==="user" ? "row-reverse" : "row" }}>
              <div style={{
                ...
                background: m.role==="user" ? "rgba(232,255,71,0.15)" : "rgba(255,255,255,0.06)",
                border: m.role==="user" ? "1px solid rgba(232,255,71,0.3)" : "1px solid rgba(255,255,255,0.1)",
              ...
              <div style={{
                ...
                background: m.role==="user" ? "rgba(232,255,71,0.07)" : "rgba(255,255,255,0.04)",
                border: m.role==="user" ? "1px solid rgba(232,255,71,0.15)" : "1px solid rgba(255,255,255,0.08)",
                fontSize:13, color: m.role==="user" ? "#D4D0C8" : "#B0ACA4",
          ...
          {chatLoading && (
            <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
              <div style={{ width:30, height:30, borderRadius:8, background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13 }}>◈</div>
              <div style={{ padding:"14px 16px", borderRadius:12, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", display:"flex", alignItems:"center", gap:10 }}>
                <Spinner small />
                <span style={{ fontSize:12, color:"#555" }}>Analyzing…</span>
              </div>
            </div>
          )}
          <div ref={bottomRef} />
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, padding:"10px 14px" }}>
        <input
          ...
          style={{ flex:1, background:"transparent", border:"none", outline:"none", color:"#D4D0C8", fontFamily:"'Syne',sans-serif", fontSize:13 }}
        />
        <button
          onClick={() => sendMessage()}
          disabled={!input.trim() || chatLoading}
          style={{ padding:"8px 16px", background:input.trim()&&!chatLoading?"#E8FF47":"rgba(255,255,255,0.06)", color:input.trim()&&!chatLoading?"#0A0A0B":"#444", border:"none", borderRadius:8, fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:700, cursor:input.trim()&&!chatLoading?"pointer":"not-allowed", letterSpacing:"0.06em", flexShrink:0, transition:"all 0.15s" }}
        >Send ↑</button>
      </div>

      {/* Quick follow-ups after first message */}
      {messages.length >= 2 && (
        <div style={{ marginTop:12, display:"flex", flexWrap:"wrap", gap:6 }}>
          {["Go deeper on this", "Give me a specific action plan", "What are the risks?", "Show me numbers"].map((q,i) => (
            <button key={i} onClick={() => sendMessage(q)} disabled={chatLoading} style={{ padding:"5px 12px", borderRadius:14, background:"transparent", border:"1px solid rgba(255,255,255,0.08)", color:"#555", fontSize:11, cursor:"pointer", fontFamily:"'Syne',sans-serif", transition:"all 0.15s" }}
            onMouseEnter={e => { e.target.style.color="#999"; e.target.style.borderColor="rgba(255,255,255,0.2)"; }}
...
Generate a realistic competitive intelligence report for that area. Return ONLY raw JSON — no markdown, no explanation, no code fences, just the JSON object starting with { and ending with }.

{
  "area_summary": {
    "location": "short neighborhood name",
    "city": "City, State",
    "total_restaurants": 42,
    "avg_rating": 4.3,
    "avg_delivery_time": "25-40 min",
    "market_temp": "Hot",
    "market_insight": "Two sentences about this area food delivery market."
  },
  "competitors": [
    {
      ...
      "price_range": "$$",
      "platform": "Both",
      "promo": "20% off first order or null",
      "top_item": "Their most popular dish",
      "threat_level": "High",
  ...
  "top_dishes": [
    {
      ...
      "price": "$18",
      "badge": "Most Ordered",
      "signal": "One sentence on why this dish is winning."
    }
  ],
  "promotions": [
    {
      "restaurant": "Restaurant Name",
      "promo_type": "Free Delivery",
      "detail": "Free delivery on orders over $15",
      "platform": "DoorDash",
      "effectiveness": "High"
    }
  ],
  "strategy": {
    ...
    "dish_opportunities": ["dish idea 1", "dish idea 2"],
    "promo_recommendation": "Specific promo recommendation with reasoning.",
    "positioning": "2-3 sentence positioning strategy."
  }
}

Include 5-6 competitors, 6-7 top_dishes, 4-5 promotions. Make it realistic for the actual location. Return ONLY the JSON, nothing else.`;

    try {
      const text = await callClaude([{ role:"user", content: prompt }], 3000);
      setRawDebug(text.slice(0, 200));
      ...
      if (!parsed) {
        try { parsed=JSON.parse(text.replace(/```json/gi,"").replace(/```/g,"").trim()); } catch {}
      }

      if (parsed && parsed.area_summary) {
        setReport(parsed);
        setRawDebug(null);
      } else {
        setError("Received a response but couldn't read the report. Debug: " + text.slice(0,200));
      }
    } catch(e) {
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    body { background:#0A0A0B; color:#D4D0C8; font-family:'Syne',sans-serif; min-height:100vh; }
    ...
    <>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"32px 16px 80px" }}>
        ...
        <div style={{ marginBottom:36 }}>
          ...
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:52, lineHeight:1, color:"#fff", letterSpacing:"0.05em" }}>
            LOCAL<span style={{ color:"#E8FF47" }}>PULSE</span>
          </div>
        ...
        <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, padding:24, marginBottom:32 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:14 }}>
            <div>
              ...
              <input value={address} onChange={e=>setAddress(e.target.value)} onKeyDown={e=>e.key==="Enter"&&analyze()} placeholder="e.g. 1234 Leetsdale Dr, Denver, CO"
                style={{ width:"100%", padding:"12px 14px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, color:"#fff", fontFamily:"'Syne',sans-serif", fontSize:14 }} />
            </div>
            <div>
          ...
          <button onClick={analyze} disabled={loading||!address.trim()} style={{ padding:"13px 26px", background:address.trim()&&!loading?"#E8FF47":"rgba(255,255,255,0.06)", color:address.trim()&&!loading?"#0A0A0B":"#444", border:"none", borderRadius:10, fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:700, cursor:address.trim()&&!loading?"pointer":"not-allowed", letterSpacing:"0.06em", textTransform:"uppercase", transition:"all 0.2s" }}>
        ...
        {error && (
          <div style={{ padding:16, background:"rgba(255,68,68,0.08)", border:"1px solid rgba(255,68,68,0.2)", borderRadius:12, color:"#FF6B6B", fontSize:13, marginBottom:12 }}>
            ⚠ {error}
            {rawDebug && <div style={{ marginTop:10, fontSize:11, color:"#555", wordBreak:"break-all", fontFamily:"monospace" }}>{rawDebug}</div>}
          </div>
        )}
        ...
        {report && !loading && (
          <div className="fade-up">
            ...
            <div style={{ background:"rgba(232,255,71,0.05)", border:"1px solid rgba(232,255,71,0.15)", borderRadius:14, padding:"18px 22px", marginBottom:24, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:16 }}>
              <div>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, color:"#fff", letterSpacing:"0.04em" }}>{report.area_summary.location}</div>
                <div style={{ fontSize:12, color:"#666" }}>{report.area_summary.city}</div>
              </div>
              {[
                { label:"Nearby", value:report.area_summary.total_restaurants },
                { label:"Avg Rating", value:`★ ${report.area_summary.avg_rating}` },
                { label:"Avg Delivery", value:report.area_summary.avg_delivery_time },
                { label:"Market", value:report.area_summary.market_temp, color:marketColor(report.area_summary.market_temp) },
              ].map(s => (
            ...
            <div style={{ display:"flex", gap:2, marginBottom:28, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, padding:4, overflowX:"auto" }}>
              {TABS.map(tab => (
                <button key={tab} onClick={()=>setActiveTab(tab)} style={{ flex:1, padding:"10px 8px", background:activeTab===tab?"#E8FF47":"transparent", color:activeTab===tab?"#0A0A0B":"#555", border:"none", borderRadius:8, fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:600, cursor:"pointer", transition:"all 0.15s", whiteSpace:"nowrap", letterSpacing:"0.04em" }}>
                      ...
                      <div key={i} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:18 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12, flexWrap:"wrap", gap:8 }}>
                          <div>
                            ...
                            <div style={{ fontSize:12, color:"#555", marginTop:2 }}>{c.cuisine}</div>
                          </div>
                          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                            <Tag>{c.platform}</Tag><Tag>{c.price_range}</Tag>
                            <Tag color={`${threatColor(c.threat_level)}22`} text={threatColor(c.threat_level)}>{c.threat_level} threat</Tag>
                          </div>
                        </div>
                        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:12 }}>
                          {[["Rating",`★ ${c.rating}`],["Delivery",c.delivery_time],["Fee",c.delivery_fee]].map(([l,v]) => (
                            <div key={l} style={{ background:"rgba(255,255,255,0.03)", borderRadius:8, padding:"10px 12px" }}>
                              ...
                              <div style={{ fontSize:10, color:"#555", textTransform:"uppercase", letterSpacing:"0.08em" }}>{l}</div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div key={i} style={{ display:"grid", gridTemplateColumns:"28px 1fr auto", alignItems:"center", gap:14, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:10, padding:"14px 16px" }}>
                        ...
                        <div>
                          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                            <span style={{ fontSize:14, fontWeight:600, color:"#fff" }}>{d.name}</span>
                            <Tag color="rgba(232,255,71,0.1)" text="#E8FF47">{d.badge}</Tag>
                          </div>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    {(report.promotions||[]).map((p,i) => (
                      <div key={i} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"16px 18px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
                        <div>
                          <div style={{ fontSize:14, fontWeight:600, color:"#fff", marginBottom:4 }}>{p.restaurant}</div>
                          <div style={{ fontSize:13, color:"#888" }}>{p.detail}</div>
                        </div>
              ...
              <div className="fade-up">
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
                  <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:20 }}>
                    ...
                    {(report.strategy?.gaps||[]).map((g,i) => (
                      <div key={i} style={{ display:"flex", gap:10, marginBottom:10, fontSize:13, color:"#999" }}><span style={{ color:"#FF4444", flexShrink:0 }}>◈</span>{g}</div>
                    ))}
                  </div>
                  <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:20 }}>
                    ...
                    {(report.strategy?.quick_wins||[]).map((w,i) => (
                      <div key={i} style={{ display:"flex", gap:10, marginBottom:10, fontSize:13, color:"#999" }}><span style={{ color:"#4CAF50", flexShrink:0 }}>◈</span>{w}</div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:20, marginBottom:14 }}>
                  ...
                  {(report.strategy?.dish_opportunities||[]).map((d,i) => (
                    <div key={i} style={{ display:"flex", gap:10, marginBottom:10, fontSize:13, color:"#999" }}><span style={{ color:"#E8FF47", flexShrink:0 }}>◈</span>{d}</div>
                  ))}
                </div>
                <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:20 }}>
